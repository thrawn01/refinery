...
func newMemberList(ctx context.Context, c config.Config) (Peers, error) {
	...
	err = retry.Until(ctx, retry.Interval(clock.Millisecond*300), func(ctx context.Context, i int) error {
		...
		if err != nil {
			return fmt.Errorf("while joining member list known peers %#v: %w", m.c.GetMemberListKnownMembers(), err)
		}
		return nil
	})

	if err != nil {
		return nil, fmt.Errorf("timed out attempting to join member list: %w", err)
	}
