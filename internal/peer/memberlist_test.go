...
func TestNewPeersMemberList(t *testing.T) {
	...
	err = retry.Until(ctx, retry.Attempts(10, 300*time.Millisecond), func(ctx context.Context, i int) error {
		peers, _ := p0.GetPeers()
		if len(peers) != 3 {
			return fmt.Errorf("expected 3 peers got '%d' instead", len(peers))
		}
		return nil
	})
	assert.NoError(t, err)
